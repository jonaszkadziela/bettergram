<?php
  ...
  {
    echo 'Wystąpił błąd podczas zapisywania oceny! Spróbuj ponownie później.';
  ...
  try
  {
    ...
    if ($result && count($result) > 0)
    {
      ...
      {
        $errors[] = 'Nie można zapisać oceny, gdyż zdjęcie zostało już ocenione!';
      }
    }
    else
    {
      $errors[] = 'Nie można zapisać oceny, gdyż zdjęcie o takim ID nie istnieje!';
    }
    ...
    if (count($errors) == 0)
    {
      ...
      $db->prepared_query('INSERT INTO photos_ratings(rating, photo_id, user_id) VALUES (?, ?, ?);', [$rating, $photo_id, $user_id]);

      $response =
      [
        'responseText' => 'Pomyślnie zapisano ocenę!',
  ...
  {
    $errors[] = 'Wystąpił błąd #' . $e->getmessage() . ' podczas zapisywania oceny! Spróbuj ponownie później.';
  }
